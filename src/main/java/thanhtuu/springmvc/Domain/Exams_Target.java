package thanhtuu.springmvc.Domain;

public class Exams_Target extends Exams_TargetKey{
	 	private Integer level1;

	    private Integer level2;

	    private Integer level3;

	    private Integer level4;

	    private Integer level5;
	    
	    private Integer level1time;

	    private Integer level2time;

	    private Integer level3time;

	    private Integer level4time;

	    private Integer level5time;
	    
	    public Integer getLevel5time() {
	        return level5time;
	    }

	    public void setLevel5time(Integer level5time) {
	        this.level5time = level5time;
	    }
	    
	    public Integer getLevel4time() {
	        return level4time;
	    }

	    public void setLevel4time(Integer level4time) {
	        this.level4time = level4time;
	    }
	    
	    public Integer getLevel3time() {
	        return level3time;
	    }

	    public void setLevel3time(Integer level3time) {
	        this.level3time = level3time;
	    }
	    
	    public Integer getLevel2time() {
	        return level2time;
	    }

	    public void setLevel2time(Integer level2time) {
	        this.level2time = level2time;
	    }
	    
	    public Integer getLevel1time() {
	        return level1time;
	    }

	    public void setLevel1time(Integer level1time) {
	        this.level1time = level1time;
	    }

	    public Integer getLevel1() {
	        return level1;
	    }

	    public void setLevel1(Integer level1) {
	        this.level1 = level1;
	    }

	    public Integer getLevel2() {
	        return level2;
	    }

	    public void setLevel2(Integer level2) {
	        this.level2 = level2;
	    }

	    public Integer getLevel3() {
	        return level3;
	    }

	    public void setLevel3(Integer level3) {
	        this.level3 = level3;
	    }

	    public Integer getLevel4() {
	        return level4;
	    }

	    public void setLevel4(Integer level4) {
	        this.level4 = level4;
	    }

	    public Integer getLevel5() {
	        return level5;
	    }

	    public void setLevel5(Integer level5) {
	        this.level5 = level5;
	    }
}
